# I made this to try to start making my own graphics/font library and it worked,
# but I gave up when i realized all the other functions I needed to implement 
# and that I could just use all caps with the existing 8 pix tall font on a 7 pix
# tall display. 
...
def render_text(text, spacing=1):
    ...
    for i in range(nchar):
        print(ord(text[i]))
        symbol = bitmap[ord(text[i])]
        for j in range(5):
            offset = i * 6 + j
            print(f"symbol[j]:{symbol[j]}")
            print(f"offset:{offset}")
            buffer.append(symbol[j])
            # print(f"buffer[offset]:{buffer[offset]}")
        for s in range(spacing):
            buffer.append(0x0)
    print(f"buffer:{buffer}")
    return buffer

# Used this in testing to show my own text with render_text. Putting this here in case I re-visit this
def show_text(scroll, bitmap):
    for x in range(len(bitmap)):
        for y in range(7):
            val = ((bitmap[x] & (0x1 << y)) >> y) * 255
            print(f"{x}, {y}, {val}")
